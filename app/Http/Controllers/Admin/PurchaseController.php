...
class PurchaseController extends Controller
{
    public function generateTicket($id_encriptado)
    {
        try {
            // Desencriptar el ID
            $purchase_id = Crypt::decrypt($id_encriptado);

            // Obtener la venta y sus detalles
            $purchase = Purchase::with('details.product', 'contact', 'branch')->findOrFail($purchase_id);
            ...
            $company = [
                ...
                'footer' => $purchase->branch->message,
            ];
            ...
            $data = [
                'company' => $company,
                'date' => $purchase->date,
                'customer' => $purchase->contact->name ?? 'Cliente Genérico',
                'items' => $items,
            ...
            $pdfView = view('admin.tickets.purchase', $data)->render();
